...
import { FileText, Loader2, Mic, MicOff, AlertTriangle, Calendar, Volume2, Square, Share2 } from 'lucide-react';
...
function App() {
  ...
  const speak = (text, lang, id) => {
    ...
    if (speakingId === id) {
      // Toggle off if already playing this block
      setSpeakingId(null);
      return;
    }
    const utter = new window.SpeechSynthesisUtterance(text);
    utter.lang = lang;
    // Try to find a voice matching the requested lang; fall back to hi-IN for Marathi
  ...
  // Detect if text is likely Devanagari (Hindi/Marathi)
  const devanagariLang = (text) => /[\u0900-\u097F]/.test(text) ? 'hi-IN' : 'en-IN';
                                ...
                                {chatHistory.map((msg, idx) => (
                                    <div key={idx} className={`flex flex-col ${msg.type === 'user' ? 'items-end' : 'items-start'}`}>
                                        <div className={`max-w-[90%] px-5 py-3 text-[14px] leading-relaxed shadow-sm ${msg.type === 'user' ? 'bg-clawde-ink text-clawde-parchment font-sans' : 'bg-white border border-clawde-ink/10 text-clawde-charcoal font-serif'}`}>
                                            ...
                                            {msg.type === 'ai' && (
                                                <div className="mt-2 pt-2 border-t border-clawde-ink/5 flex items-center gap-3">
                                                    {msg.source === 'general' ? (
                                                    ...
                                                    {hasTTS && (
                                                        <button
                                                            onClick={() => speak(msg.text, devanagariLang(msg.text), `chat_${idx}`)}
                                                            className="ml-auto p-0.5 text-clawde-ink/25 hover:text-clawde-ink transition-colors"
                                                            title={speakingId === `chat_${idx}` ? 'Stop' : 'Listen'}
                                                        >
                                                            {speakingId === `chat_${idx}` ? <Square className="w-3 h-3" /> : <Volume2 className="w-3 h-3" />}
                                                        </button>
                                                    )}
                                                </div>
                                            )}
